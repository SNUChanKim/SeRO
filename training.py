    ...
    for i_episode in itertools.count(1):
        ...
        while not done:
            ...
            if args.start_steps > total_num_steps:
                ...
                deg_uncertainty = 0
                writer.add_scalar('deg_uncertainty', deg_uncertainty, total_num_steps)
                writer.add_scalar('std', std.mean(), total_num_steps)
                
                if len(memory) > args.batch_size:
                    for i in range(args.updates_per_step):
                        train_start = time.time()
                        critic_loss, policy_loss, entropy_loss, total_loss = agent.update_parameters(memory, args.batch_size, updates)
                        train_end = time.time()
            ...
            next_deg_uncertainty = 0
            
            if args.policy == 'sero':
                if args.uncertainty_type == 'scalar':
                    aux_reward = -next_deg_uncertainty*args.aux_coef
                else:
                    raise NotImplementedError
            else:
                aux_reward = 0
